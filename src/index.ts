...
app.get("/health", (_req, res) => {
  res.json({ ok: true, service: "TinkerMCP", version: "0.5.0" });
});
...
app.post("/remote/navigate", async (req, res) => {
  try {
    res.json(await remoteNavigate(String(req.body?.url || "https://www.tinkercad.com/")));
  } catch (error) {
    res.status(500).json({ ok: false, error: error instanceof Error ? error.message : String(error) });
  }
});
...
app.post("/remote/click", async (req, res) => {
  try {
    res.json(await remoteClick(Number(req.body?.x), Number(req.body?.y)));
  } catch (error) {
    res.status(500).json({ ok: false, error: error instanceof Error ? error.message : String(error) });
  }
});
...
app.post("/remote/type", async (req, res) => {
  try {
    res.json(await remoteType(String(req.body?.text ?? "")));
  } catch (error) {
    res.status(500).json({ ok: false, error: error instanceof Error ? error.message : String(error) });
  }
});
...
app.post("/remote/key", async (req, res) => {
  try {
    res.json(await remoteKey(String(req.body?.key || "Enter")));
  } catch (error) {
    res.status(500).json({ ok: false, error: error instanceof Error ? error.message : String(error) });
  }
});
...
function refresh(){screen.src='/remote/screenshot.jpg?t='+Date.now();fetch('/remote/status').then(r=>r.json()).then(s=>{status.textContent=(s.title||'')+' — '+(s.url||'');if(s.url)document.getElementById('url').value=s.url}).catch(()=>{});}
async function nav(){await api('/remote/navigate',{url:document.getElementById('url').value});refresh()}
