...
import { remoteDrag } from "./drag.js";

function createMcpServer() {
  const server = new McpServer({ name: "TinkerMCP", version: "0.5.2" });
...
app.get("/health", (_req, res) => res.json({ ok: true, service: "TinkerMCP", version: "0.5.2" }));
...
app.post("/remote/drag", async (req, res) => {
  try {
    res.json(await remoteDrag(
      Number(req.body?.fromX),
      Number(req.body?.fromY),
      Number(req.body?.toX),
      Number(req.body?.toY),
      Number(req.body?.steps || 24),
    ));
  } catch (error) { res.status(500).json({ ok: false, error: error instanceof Error ? error.message : String(error) }); }
});
...
<div class="wrap"><img id="screen" class="screen" src="/remote/screenshot.jpg?t=0" alt="browser" draggable="false"><br><small>Click normal para hacer click. Para arrastrar: mantené apretado sobre la imagen, mové y soltá.</small></div>
<script>
...
function toRemote(e){const r=screen.getBoundingClientRect();return {x:(e.clientX-r.left)*screen.naturalWidth/r.width,y:(e.clientY-r.top)*screen.naturalHeight/r.height};}
function refresh(){
...
screen.addEventListener('mousedown',e=>{e.preventDefault();dragStart=toRemote(e);});
screen.addEventListener('mouseup',async e=>{
  if(!dragStart)return;
  e.preventDefault();
  const end=toRemote(e);
  const dx=end.x-dragStart.x,dy=end.y-dragStart.y;
  const dist=Math.hypot(dx,dy);
  try{
    if(dist<5){await api('/remote/click',{x:end.x,y:end.y});}
    else{await api('/remote/drag',{fromX:dragStart.x,fromY:dragStart.y,toX:end.x,toY:end.y,steps:30});}
  }finally{dragStart=null;setTimeout(refresh,250);}
});
screen.addEventListener('mouseleave',()=>{dragStart=null;});
