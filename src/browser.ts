...
export async function inspectEditor(): Promise<unknown> {
  ...
  return p.evaluate(() => {
    ...
    const globals = Object.keys(window)
      .filter((key) => /tinker|editor|design|shape|workspace|scene|redux|store|three/i.test(key))
      .slice(0, 200);
...
export async function diagnosticSnapshot(): Promise<unknown> {
  ...
  const browserState = await p.evaluate(() => {
    ...
    return {
      url: location.href,
      title: document.title,
      events: [...(w.__tinkerMcpEvents ?? [])],
      mutations: [...(w.__tinkerMcpMutations ?? [])],
    };
  });
